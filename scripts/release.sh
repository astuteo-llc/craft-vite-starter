...
I_OK="✅"; I_STOP="🚫"; I_ERROR="❌"; I_END="👋🏻"; I_QUESTION="❓";
...
do-push() {
  ...
  if [ "$FLAG_PUSH" = true ]; then
    ...
    echo -ne "\n${I_QUESTION} ${S_QUESTION}DEPLOY by pushing tag now?<${S_NORM}${PUSH_DEST}${S_QUESTION}>? [${S_NORM}N/y${S_QUESTION}]: "
    read CONFIRM
  fi
...
echo -e "\n${I_OK} ${S_NOTICE}Added GIT tag"
  case "$CONFIRM" in
    [yY][eE][sS]|[yY] )
      ...
      PUSH_MSG=`git push && git push --tags 2>&1` # Push new tag
      if [ ! "$?" -eq 0 ]; then
